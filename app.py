...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "key_1234")  # Cambia esta clave en producción
...
def get_db_connection():
    try:
        ...
        # Aquí logueamos la causa y relanzamos para que Flask lo capture
        app.logger.error(f"No pude conectar a MySQL: {e}")
        raise

def authenticate_user(email, password):
    """  
    Devuelve (id, nombre, rol) si las credenciales son correctas, 
    o None en caso contrario.
    """
    SECRET = os.environ.get("AES_KEY", "mi_clave_secreta")
    sql = """
        SELECT 
          id,
          CAST(AES_DECRYPT(password, %s) AS CHAR) AS pwd,
          CAST(AES_DECRYPT(nombre,   %s) AS CHAR) AS nombre,
          CAST(AES_DECRYPT(rol,      %s) AS CHAR) AS rol
        FROM usuarios
        WHERE CAST(AES_DECRYPT(email, %s) AS CHAR) = %s
    """
    params = (SECRET, SECRET, SECRET, SECRET, email)
    try:
        ...
        print("Error en authenticate_user (BD):", e)
        return None
    ...
    # db_pwd, db_name y db_role ya son str (gracias al CAST)
    if db_pwd == password:
