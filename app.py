...
def get_db_connection():
    ...
    return pymysql.connect(
        ...
        charset="utf8mb4",
        connect_timeout=5,
        cursorclass=pymysql.cursors.DictCursor,  # Opcional: para devolver dicts en lugar de tuplas
        autocommit=True                        # Opcional: para no tener que llamar a conn.commit() cada vez
    )
...
def authenticate_user(email, password):
    try:
        ...
        sql = """
            SELECT id, AES_DECRYPT(password, 'mi_clave_secreta') as pwd,
                   AES_DECRYPT(nombre, 'mi_clave_secreta') as nombre,
                   AES_DECRYPT(rol, 'mi_clave_secreta') as rol
            FROM usuarios
            WHERE email = AES_ENCRYPT(%s, 'mi_clave_secreta')
        """
        ...
        connection.close()
        if result:
            user_id, db_pwd, db_name, db_role = result
            if db_pwd is not None:
                db_pwd = db_pwd.decode('utf8')
            if db_name is not None:
                db_name = db_name.decode('utf8')
            if db_role is not None:
                db_role = db_role.decode('utf8')
            if db_pwd == password:
