...
def authenticate_user(email, password):
    try:
        ...
        sql = """
            SELECT 
              id, 
              AES_DECRYPT(password, 'mi_clave_secreta') AS pwd,
        ...
        # Si viene como bytes, decodificamos; si ya es str, lo dejamos
        if isinstance(db_pwd, (bytes, bytearray)):
            db_pwd = db_pwd.decode('utf-8')
        if isinstance(db_name, (bytes, bytearray)):
            db_name = db_name.decode('utf-8')
        if isinstance(db_role, (bytes, bytearray)):
            db_role = db_role.decode('utf-8')
